  ...
  (let ((icount 0))
    (define (resolve var env)
      (cond ((assq var env) => cdr)
	    (else var)))
    (define (substitutable? x)
      (match x
	((? symbol?) #t)
	(('quote _) #t)
	(_ #f)))
    (define (substitute x env)
      (cond ((symbol? x) (resolve x env))
	    ((or (boolean? x) (number? x) (char? x) (string? x) (vector? x)) `(quote ,x))
	    (else x)))
    (define (walk x env)
      (match x
	...
	((('lambda llist body ...) args ...)
	 (inc! icount)
	 (let loop ((vars llist) (args (map (cut substitute <> env) args)) (bs '()) (ss '()))
	   (cond ((null? vars)
		  (if (null? args)
		      (walk `(let ,(reverse bs) ,@body) (append ss env))
		      (error "too many arguments in manifest `lambda' call" x)))
		 ((symbol? vars)
		  (walk `(let ,(append (reverse bs) `((,vars (%list ,@args)))) ,@body) (append ss env)))
		 ((null? args)
		  (error "too few arguments in manifest `lambda' call" x))
		 ((pair? vars)
		  (if (substitutable? (car args))
		      (loop (cdr vars) (cdr args) bs (alist-cons (car vars) (car args) ss))
		      (loop (cdr vars) (cdr args) (cons (list (car vars) (car args)) bs) ss)))
		 (else (error "invalid lambda list" llist)))))
	((('$case-lambda ('lambda llists . bodies) ...) args ...)
	 ...
	 (let loop1 ((llists llists) (bodies bodies))
	   ...
	   (let loop2 ((vars (car llists)) (args (map (cut substitute <> env) args)) (bs '()) (ss '()))
	     (cond ((null? vars)
		    (if (null? args)
			(walk `(let ,(reverse bs) ,@(car bodies)) (append ss env))
			(loop1 (cdr llists) (cdr bodies))))
		   ((symbol? vars)
		    (walk `(let ,(append (reverse bs) `((,vars (%list ,@args)))) ,@(car bodies)) 
			  (append ss env)))
		   ((null? args)
		    (loop1 (cdr llists) (cdr bodies)))
		   ((pair? vars)
		    (if (substitutable? (car args))
			(loop2 (cdr vars) (cdr args) bs (alist-cons (car vars) (car args) ss))
			(loop2 (cdr vars) (cdr args) (cons (list (car vars) (car args)) bs) ss)))
