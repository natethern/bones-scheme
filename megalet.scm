;;;; "megalet" - SRFI-71 + extensions (see sections marked "(flw)"
...
;   wrapping the skeletons around the payload (let (vx ...) . body).
    ...
    ;; handle "implicit nested" lambdas (flw)
    ((i:let "bs" rec cwvs vxs body ((((hd . tl) . llist) body2 ...) bs ...))
     (i:let "bs" rec cwvs vxs body (((hd . tl) (lambda llist body2 ...)) bs ...)))

    ;; handle "implicit" lambdas (flw)
    ((i:let "bs" rec cwvs vxs body (((name . llist) body2 ...) bs ...))
     (i:let "bs" rec cwvs vxs body ((name (lambda llist body2 ...)) bs ...)))
...
(define-syntax define
  (let-syntax ((old-define define))
    (syntax-rules ()
      ((_ ((hd . tl) . llist) body ...)
       (define (hd . tl) (lambda llist body ...)))
      ((_ args ...)
       (old-define args ...)))))
