  ...
  (cond ((fixnum? c) (generate-equ l "(" c " << 1) | 1"))
	...
	((bytevector? c)
	 ...
	 (let ((len (bytevector-length c))
	   ...
	   (generate-defword "BYTEVECTOR | " len)
	   (do ((i 0 (add1 i))
		(j 0 (remainder (add1 j) 40)))
	       ((>= i len)
		(let ((str (get-output-string out)))
		  (unless (string=? "" str)
		    (generate-defbyte str))))
	     (unless (zero? j) (write-char #\, out))
	     (display (bytevector-u8-ref c i) out)
	     (when (zero? j)
	       (generate-defbyte (get-output-string out))
